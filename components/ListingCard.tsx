...
interface ListingCardProps {
  ...
  context?: "myProperty" | "myReservation" | "reservationReceived" | "default";
  onClick?: () => void;
}
    ...
    <>
      ...
      <div className="flex flex-col gap-1 w-full">
        ...
        <div className="flex flex-row items-baseline gap-1">
          ...
          {!reservation && <span className="font-light">hora</span>}
        </div>
      </div>
    </>
  );

  if (context !== "default" && onClick) {
    return (
      <div className="relative">
        <div
          onClick={handleClick}
          className="col-span-1 cursor-pointer hover:opacity-80 transition"
        >
          {content}
        </div>
      </div>
